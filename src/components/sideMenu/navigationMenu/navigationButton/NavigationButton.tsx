...
function joinPathSegments(basePath: string, nextSegment?: string): string {
  ...
  const right = nextSegment.startsWith("/")
    ? nextSegment.slice(1)
    : nextSegment;
  return `${left}/${right}`;
}
...
export default function NavigationButton({
  ...
  useEffect(() => {
    setButtonLabel(formatButtonLabel(navigationTarget) ?? "HOME");
  }, [knownFrameOrigins, navigationTarget]);
  ...
  function buildRelativeDestinationPath(target: NavigationTarget): string {
    ...
    if (target.pageName)
      path = joinPathSegments(path, encodeURIComponent(target.pageName));
    return path;
  }
